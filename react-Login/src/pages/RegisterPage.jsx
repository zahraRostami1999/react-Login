            ...
            <div className='bg-pink-300 h-screen flex justify-center items-center font-sans'>
                <div className='md:w-2/3 lg:w-1/3 bg bg-black-opacity-5 lg:h-2/3 rounded sm:w-5/6 sm:h-5/6 md:h-5/6'>
                    ...
                    <div className="grid grid-rows-9 grid-cols-1 mx-10 rounded mt-10">
                        <input onChange={(e) => setUser(e.target.value)} className="rounded-xl px-2 h-15 border-none text-base" type='text' placeholder='Username' />
                        <p>{validUser ? "Okay✅" : "❌Your username must be more than 3 characters."}</p>
                        <input onChange={(e) => setPwd(e.target.value)} className="rounded-xl px-2 h-15 border-none text-base" type='password' placeholder='Password' />
                        <p>{validPwd ? "Okay✅" : "❌Your password must contain at least 4 characters and one number."}</p>
                        <input onChange={(e) => setMatchPwd(e.target.value)} className="rounded-xl px-2 h-15 border-none text-base" type='password' placeholder='Confirm Password' />
                        <p>{validMatchPwd ? "Okay✅" : "❌You must re-enter your password."}</p>
                        <button className="bg-orange-600 rounded-xl" onClick={()=>setNewUser()}>Sign Up</button>
